...
class Umbrella:
    ...
    def __init__(self, rag: RAG, llmjudge: LLMJudge, relevant_threshold: float):
    ...
    def evaluate_query(self, query: str) -> UmbrellaMetrics:
        ...
        scores = self.llmjudge.judge_query(query, context)
        return self._scores_to_metrics(scores)

    def _scores_to_metrics(self, scores: List[int]) -> UmbrellaMetrics:
        """Convert raw relevance scores into umbrella metrics."""
        binary_relevance = [
            1 if score >= self.relevant_threshold else 0
            for score in scores
        ]
        return UmbrellaMetrics(
